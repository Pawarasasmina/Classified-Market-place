...
function SellerWalletBalance({
  ...
  const recentLedger =
    wallet.ledger?.slice(0, walletActivityPreviewLimit) ?? [];
    ...
    <section className="panel grid gap-5 lg:grid-cols-[1fr_1.35fr] lg:items-start">
      <div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <Link
            href="/sell"
            className="action-primary px-3 py-2 text-sm font-black"
          >
            Create listing
          </Link>
        </div>
...
function SellerBoostOptions({
  ...
  const walletReadyCount = optionSummaries.filter(
    (option) => option.walletAffordable && option.eligibleListings.length > 0,
  ).length;
                  ...
                  {listing.isBoosted ? (
                    <p className="mt-2 text-sm font-semibold text-[var(--success)]">
                      ...
                      <span className="block text-xs text-[var(--muted)]">
                        ...
                        boosted inquiries / {listing.boostConversionRate}% boost
                        conversion
                      </span>
                    </p>
                  ) : null}
